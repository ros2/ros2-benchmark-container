...
for RMW in "${RMW_LIST[@]}"; do
  ...
  for COMM in "${COMMS[@]}"; do
    ...
    for i in "${!TOPOLOGY1[@]}"; do
      ...
      COMMAND="${IROBOT_BENCHMARK} ${TOP1_PATH} ${TOP2_PATH} --executor ${EXECUTOR_ARG} ${THREADS_OPTION} --ipc off -t ${ROS2_BENCHMARK_TEST_DURATION} -s 1000 --csv-out on --results-dir ${RESULT_FOLDER      echo -e "     Command: \n       $COMMAND"
      eval "$COMMAND"
      benchmark_exit_code=$?
